...
SPARK_HISTORY_OPTS="-Dspark.ui.filters=yjava.servlet.filter.BouncerFilter -Dspark.history.kerberos.enabled=true -Dspark.history.kerberos.principal=${spark_kerberos_principal} -Dspark.history.kerberos.keytab=${spark_kerberos_keytab} -Dspark.history.ui.acls.enable=true -Dspark.authenticate=false -Dspark.ui.acls.enable=false -Dspark.history.retainedApplications=${spark_history_retained_applications} -Dspark.history.fs.cleaner.enabled=${spark_history_cleaner_enable} -Dspark.history.fs.cleaner.maxAge=${spark_history_cleaner_max_age} -Dspark.history.fs.cleaner.interval=${spark_history_cleaner_interval} ${spark_history_bouncer_filter_params} -Dspark.history.ui.maxApplications=${spark_history_history_maxApplications} -Dspark.ssl.historyServer.enabled=${spark_history_ssl_enabled} -Dspark.ssl.historyServer.keyPassword='password' -Dspark.ssl.historyServer.keyStore=${spark_history_ssl_keystore}  -Dspark.ssl.historyServer.keyStorePassword='password' -Dspark.ssl.historyServer.trustStore=${spark_history_ssl_truststore}  -Dspark.ssl.historyServer.trustStorePassword='password' -Dspark.ssl.historyServer.needClientAuth=true -Dspark.ssl.protocol='TLSv1.2' -Dspark.ssl.historyServer.port=${spark_history_ssl_port}"
